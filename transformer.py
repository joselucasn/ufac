...
MAPA_ETAPA_KANBAN = {
    ...
    "APROVADOS AGUARDANDO LIBERAÇÃO FISCALIZAÇÃO": "FISCALIZAÇÃO",
    # ── Em Execução ──
    "PARA EXECUÇÃO":                     "EM EXECUÇÃO",
    "ATRASADAS":                         "EM EXECUÇÃO",
    "EM EXECUÇÃO":                       "EM EXECUÇÃO",
    # ── Concluída ──
    "FINALIZADAS EM FECHAMENTO DE MEDIÇÃO": "CONCLUÍDA",
...
def categorizar_status_kanban(os: dict) -> str:
    ...
    if "APROVADO" in etapa_norm or "LIBERAÇÃO" in etapa_norm or "LIBERACAO" in etapa_norm or "FISCALIZAÇÃO" in etapa_norm or "FISCALIZACAO" in etapa_norm:
        return "FISCALIZAÇÃO"
    ...
    if "LEVANTAMENTO" in etapa_norm:
        return "EM ABERTO"
    if "FECHAMENTO" in etapa_norm or "PAGAMENTO" in etapa_norm or "MEDIÇÃO" in etapa_norm or "MEDICAO" in etapa_norm:
        return "CONCLUÍDA"
...
# ── CRITÉRIOS DE EXPORTAÇÃO PARA MEDIÇÃO ──────────────────
# Estados que entram na planilha de estimativa de pagamento
# O fiscal precisa saber o valor total necessário para fechar o pagamento da medição

ESTADOS_EXPORTACAO_MEDICAO = [
    "FINALIZADAS EM FECHAMENTO DE MEDIÇÃO",
    "PARA PAGAMENTO",
    "ELABORAR CONFORMIDADE",
    "ATRASADAS",
...
def filtrar_para_medicao(tarefas: list[dict]) -> list[dict]:
    """
    Filtra O.S. que impactam o fechamento de medição.
    Critérios:
      - Possui valor total > 0 (orçamento inicial realizado e aprovado)
      - Está em um dos estados: FINALIZADAS EM FECHAMENTO DE MEDIÇÃO,
        PARA PAGAMENTO, ELABORAR CONFORMIDADE, ATRASADAS, PARA EXECUÇÃO
    """
    resultado = []
    for os in tarefas:
        # 1. Deve ter valor (orçamento aprovado)
        valor = extrair_valor_total(os)
        if not valor or valor <= 0:
            continue

        # 2. Estado deve estar na lista de exportação
        estado = str(os.get("Estado", "")).strip().upper()
        if estado not in [e.upper() for e in ESTADOS_EXPORTACAO_MEDICAO]:
            continue

        resultado.append(os)

    # Ordena por estado (fluxo lógico) e depois por entrega desejada
    ordem_estados = {e.upper(): i for i, e in enumerate(ESTADOS_EXPORTACAO_MEDICAO)}

    def _sort_key(item):
        est = str(item.get("Estado", "")).strip().upper()
        ordem = ordem_estados.get(est, 99)
        entrega = str(item.get("Entrega desejada", "")).strip()[:10]
        return (ordem, entrega or "9999-12-31")

    resultado.sort(key=_sort_key)
    return resultado


def gerar_csv_medicao(tarefas: list[dict]) -> str:
    """
    Gera CSV para exportação de medição.
    Colunas: ID, Título, Contrato, Local, Estado, Valor Total,
             Mão de Obra, Material, Entrega Desejada, Competência,
             Fiscal Responsável, Etapa
    """
    import csv, io
    ...
    # Cabeçalho
    writer.writerow([
        "ID", "Título", "Contrato", "Local do Serviço", "Estado",
        "Valor Total (R$)", "Mão de Obra (R$)", "Material (R$)",
        "Entrega Desejada", "Competência", "Fiscal Responsável", "Etapa"
    ])

    for os in tarefas:
        valor = extrair_valor_total(os) or 0
        mo = extrair_valor_mo(os) or 0
        ma = extrair_valor_ma(os) or 0

        writer.writerow([
            os.get("ID", ""),
            str(os.get("Titulo", os.get("title", "")))[:100],
            os.get("Contrato", ""),
            os.get("Local do servico", ""),
            os.get("Estado", ""),
            f"{valor:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."),
            f"{mo:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."),
            f"{ma:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."),
            str(os.get("Entrega desejada", ""))[:10],
            os.get("Competencia", ""),
            os.get("Fiscal responsavel", ""),
            os.get("Etapa", ""),
        ])
