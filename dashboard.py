...
/* ── Card expandido compacto ── */
.os-expanded {
    font-size: 13px !important;
    line-height: 1.3 !important;
}
.os-expanded .item-row {
...
.os-expanded .doc-row a:hover {
    background: rgba(88,166,255,0.2) !important;
}
.os-expanded .fin-grid {
    display: flex !important;
    gap: 6px !important;
    margin: 6px 0 !important;
}
.os-expanded .fin-item {
...
.os-expanded .stLinkButton a {
    padding: 4px 12px !important;
    font-size: 12px !important;
    height: auto !important;
    min-height: 0 !important;
}
.os-expanded div[data-testid="stHorizontalBlock"] {
    gap: 0.3rem !important;
}
.os-expanded .fin-item .txt-verde { color: #3fb950 !important; }
.os-expanded .fin-item .txt-azul { color: #58a6ff !important; }
.os-expanded .fin-item .txt-roxo { color: #bc8cff !important; }
...
def render_os_card(os: dict):
    """Card Kanban com expander compacto"""
    os_id = os.get("ID", "")
    ...
    with st.expander(label, expanded=False):
        st.markdown('<div class="os-expanded">', unsafe_allow_html=True)
        ...
        docs_links = []
        for lbl, url in [("📋 Relat. Final", link_fin), ("📊 Orç. Final", link_orcf)]:
            if url and url not in ("", "-", "#N/A"):
                docs_links.append(f'<a href="{url}" target="_blank">{lbl}</a>')
        if docs_links:
            st.markdown(f'<div class="doc-row">{" ".join(docs_links)}</div>', unsafe_allow_html=True)

        # ── Campos em linha única ──
        st.markdown(f'<div class="item-row">📋 **Etapa:** {etapa or "—"}</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="item-row">🏢 **Local:** {local or "—"}</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="item-row">👤 **Fiscal:** {responsavel or "—"}</div>', unsafe_allow_html=True)
        if solicitante:
            st.markdown(f'<div class="item-row">👤 **Solicitante:** {solicitante}</div>', unsafe_allow_html=True)

        # ── Valores financeiros em grid 3 colunas ──
        vt = _fmt_brl(os.get('Valor total', ''))
        mo = _fmt_brl(os.get('Valor M.O', ''))
        ma = _fmt_brl(os.get('Valor M.A', ''))
        clr_vt = "txt-verde" if "R$" in vt else ""
        clr_mo = "txt-azul" if "R$" in mo else ""
        clr_ma = "txt-roxo" if "R$" in ma else ""
        st.markdown(f'''
        <div class="fin-grid">
            <div class="fin-item">
                <div class="fin-label">💰 Total</div>
                <div class="fin-value {clr_vt}">{vt}</div>
            </div>
            <div class="fin-item">
                <div class="fin-label">👷 M.O.</div>
                <div class="fin-value {clr_mo}">{mo}</div>
            </div>
            <div class="fin-item">
                <div class="fin-label">📦 M.A.</div>
                <div class="fin-value {clr_ma}">{ma}</div>
            </div>
        </div>
        ''', unsafe_allow_html=True)

        # Link guest compacto
        st.link_button("🔗 Abrir no Runrun.it", guest_url, use_container_width=True)
...
def aplicar_filtros(tarefas: list, contratos, status, local, dt_ini, dt_fim, fiscal=None, processo="") -> list:
    filtradas = []
    for os in tarefas:
        # Filtro fiscal
...
def _fmt_brl(v):
    ...
    if v is None or str(v).strip() in ("", "-", "#N/A", "0"):
        return "—"
    try:
